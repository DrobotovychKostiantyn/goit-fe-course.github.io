...
const galleryItems = [
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const createFullview = ({fullview,alt}) => {
      ...
      <img src="${fullview}" alt="${alt}" class="fullImg" width=1200 height=500>
    </div>`
    };
  
    const previewLi = galleryItems.reduce((acc,{preview,fullview,alt}) =>acc + `<li><img src="${preview}" class="image" data-fullview="${fullview}" alt="${alt}" width=200 height=150></li>`,'');
    const createPreview = () => {
